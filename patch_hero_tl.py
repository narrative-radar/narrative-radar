...
def patch_hero_tl():
    ...
    # 1. FIX THE TIMELINE LINE ISSUE
    tl_old = """<div class="grid grid-cols-1 md:grid-cols-4 gap-8 pt-8 relative">
      <div class="hidden md:block absolute left-0 right-0 top-[34px] h-[1px] bg-[var(--rule)]"></div>"""
      
    tl_new = """<div class="grid grid-cols-1 md:grid-cols-4 gap-8 pt-10 relative">
      <div class="hidden md:block absolute left-0 right-0 top-[12px] h-[1px] bg-[var(--rule)]"></div>"""
      
    content = content.replace(tl_old, tl_new)
    
    # Update the dots
    content = content.replace('md:-top-[41px]', 'md:-top-[34px]')
    content = content.replace('ring-[var(--bg)]', 'ring-[var(--ink)]') # use ink for background ring

    # 2. REWRITE HERO TO MATCH PARE'S HTML COMPACTNESS & GLOW
    # First, let's find the Pare-style Hero Section
    ...
    if start_hero != -1 and end_hero != -1:
        new_hero = """<!-- Pare-style Hero Section -->
  <div class="max-w-[1344px] mx-auto px-6 pt-16 md:pt-24 pb-20 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-stretch relative z-10">
    ...
    <div class="flex flex-col">
      ...
      <div class="grid grid-cols-3 gap-3 mt-auto pt-6">
    ...
    <div class="relative flex flex-col bg-[var(--panel)] rounded-[16px] p-6 shadow-[0_30px_80px_rgba(0,0,0,.45),0_0_0_1px_rgba(60,227,167,.10)] border border-white/5">
      ...
      <a href="/radar" class="mt-auto h-[44px] rounded-xl bg-[var(--live)] hover:bg-[#56ECB6] text-[#06130D] font-semibold flex items-center justify-center transition-colors">
        View Full Terminal
      </a>
    </div>
  </div>\n  """
        content = content[:start_hero] + new_hero + content[end_hero:]
